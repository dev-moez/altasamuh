<div>
    <div class="flex flex-wrap gap-3 mt-4">
        @forelse($miscDonations as $donation)
            <button type="button" wire:click.prevent="$set('misc_donation_id', {{ $donation->id }} )" class="px-5 py-2 text-md rounded-md font-bold {{ $misc_donation_id == $donation->id ? 'bg-green-500 text-white' : 'text-gray-800 bg-gray-200' }}">{{ $donation->title }}</button>
        @empty
        @endforelse
    </div>
    {{-- Make donation  --}}
    <div class="mt-5">
        ...
        <div class="flex items-center justify-between w-full gap-4 mt-5">
            <div class="grid w-full grid-cols-1 lg:grid-cols-2 gap-x-4">
    ...
    <div class="flex flex-col flex-grow gap-4 my-4 lg:flex-row" wire:ignore>
        ...
        <x-primary-button wire:click.prevent="addToCart" class="flex-1 w-full lg:w-auto bg-[#DCDCDC] text-[#3F3F3F] gap-x-2 hover:bg-gray-400">
